...
| `/demo-4` | One **row per firm** (directory) | Ranked browse: ratings, platforms, allocation, promo. |
| `/compare-page-2` | One **row per firm** (overview table) | Roll up sizes, eval, drawdown, payout cadence. **View firm** → PFG firm page. |
| `/compare-firms` | Two firms, pick plans | Head-to-head with per-row highlights. |

`/` and `/demo-2` redirect to `/challenges`. Live allow-list lives in `src/proxy.js` (Next.js 16 — this repo uses `proxy.js`, not `middleware.js`).
...
- On Overview (`/compare-page-2`), pin **View firm** to `https://propfirmgenie.com/firm/{slug}` from `src/lib/firmGenie.js`. Not the affiliate / KAGE checkout URL.
...
src/app/demo-4/page.js
src/app/compare-page-2/page.js
src/app/compare-firms/page.js
src/proxy.js
```
...
Spotlight two names → link to `/compare-firms?...`.
...
- [ ] `/compare-page-2` — no Price pin; **View firm** on Apex opens `https://propfirmgenie.com/firm/apex`.
- [ ] Overview payout: Apex = `5 winning days` (not `$100 · $200 · $250…`). Tradeify / FundedNext are short cadence lists, not a paragraph dump.
- [ ] `/demo-4` directory still lists firms.
- [ ] `/compare-firms` still highlights two picked plans.
